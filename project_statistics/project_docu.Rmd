...
#### Wojciech Kaźmierczak, Marcin Serafin
...
-   Porównanie dla Rosenbrocka
    ...
    ```         
    Wykres jest podobny do tego z przypadku 2 wymiarowego jednak różnice między PRS i GA są większe ,a za to całkiem inny od wykresu dla funkcji Ackley'a dla 10 wymiarów.
    ```
    ...
    ```         
    Sytuacja analogiczna jak dla mniejszych wymiarów, z jeszcze wiekszą różnicą pomiędzy algorytmami.
    ```
...
Z wszystkich wykonanych testów dla dwóch próbek (Welch Two Sample t-test) wszystkie testy pozwoliły na odrzucenie hipotezy zerowej, a zatem przyjęcie hipotezy alternatywnej, iż 0 nie jest różnicą wartości oczekiwanych (średnich). W pojedyńczych testach (One Sample t-test) dla algorytmu genetycznego p-value wynosiło ponad 0.05 (w dwóch przypadkach), zatem nie odrzuciliśmy hipotezy zerowej stanowiącej, iż wartość oczekiwana(średnia) jest równa 0. Dla reszty tego typu testów a w szczególości dla wszystkich testów algorytmu PRS otrzymaliśmy p-value mniejsze od 0.05, zatem hipotezę zerową odrzuciliśmy na rzecz hipotezy alternatywnej stanowiącej, iż wartość oczekiwana (średnia) nie jest równa 0.

Jeśli chodzi o wartości otrzymane przez algorytmy, to te uzyskano po działaniu PRS mają znacznie większy rozstęp międzykwartylowy natomiast te uzyskane dzięki GA są dużo bardziej skupione przy jednej wartości. Taki wynik algorytmów zgadza się z intuicją uwzględniając ich sposób działania.

Z wykresów oraz obliczeń wynika że "lepszą" metodą jest algorytm gentyczny. Gdyż w odróżnieniu od PRS (Pure Random Search) poprzez swoją specyfikę działania sukcesywnie idąć po malejących wartościach z wyniki otrzmene dzięki niemy są bliższe tym domniemanym, oczekiwanym. PRS działa zupełnie losowo dlatego też jest bardziej podatny na większe pomyłki co do domniemanego minimum globalnego.
